  ...
  if(toggleRows && priceTableBody){
    ...
    toggleRows.addEventListener('click', function(){
      ...
      if(isOpen){
        ...
        extraRows.forEach(function(row, i){
          // znikają w odwrotnej kolejności — od ostatniego do pierwszego
          var delay = prefersReduced ? 0 : (n - 1 - i) * COLLAPSE_STAGGER;
          row.style.transitionDelay = delay + 's';
          if(prefersReduced){
            row.classList.remove('pre-show', 'show');
          } else {
            row.classList.remove('show');
            // dopiero gdy TEN wiersz skończy blaknąć, wyjmij go z układu —
            // dzięki temu tabela kurczy się na bieżąco, nie jednym skokiem na końcu
            var onFade = function(e){
              if(e.propertyName !== 'opacity') return;
              row.classList.remove('pre-show');
              row.style.transitionDelay = '0s';
              row.removeEventListener('transitionend', onFade);
            };
            row.addEventListener('transitionend', onFade);
          }
        });
      }
    });
